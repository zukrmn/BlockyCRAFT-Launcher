...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  invoke: async (channel: string, data: unknown): Promise<unknown> => {
    const validChannels = ['get-minecraft-path', 'get-java-version', 'launch-game', 'check-custom-instance', 'check-update-status', 'perform-update', 'open-external', 'kill-game', 'get-mods', 'toggle-mod', 'fetch-url', 'get-overlay-hotkey', 'set-overlay-hotkey', 'get-available-hotkeys', 'install-launcher-update'];
    if (validChannels.includes(channel)) {
